...
*   [x] **Fan RPM via SMC** — shipped v1.2.2. `smc.py` discovers per-fan actual-RPM keys (`F{n}Ac`, confirmed `flt ` type on Apple Silicon — not `fpe2` as originally guessed — count from `FNum`), `SMCReader.read_fan_rpms()`/`fan_available` mirror the temperature reader, and the TUI's "Fan" row hides entirely on fanless Macs via the `bandwidth_available` hide-row pattern. See `docs/DESIGN-system.md` §3.5.
