...
## Goal
Improve `asitop` into a cleaner, testable, CLI-friendly all-in-one utility structure while preserving current behavior and output.
...
- [ ] Replace wildcard import in `asitop/asitop.py` (`from .utils import *`) with explicit imports.
- [ ] Keep UI/render loop logic in `asitop/asitop.py` and isolate system/process helpers in `asitop/utils.py`.
...
- [ ] Keep `console_scripts` entrypoint for `asitop`.
...
- [ ] Re-run full test suite and smoke checks:
  - [ ] `.venv/bin/python -m asitop.asitop --help`
  - [ ] `.venv/bin/python -m pytest -q`
  - [ ] `sudo asitop --interval 1 --avg 30 --power-scale profile` (manual runtime check on Apple Silicon)
